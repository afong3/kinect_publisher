...
int main(int argc, char** argv)
{
    ...
    while (ros::ok() && !protonect_shutdown && (framemax == (size_t)-1 || framecount < framemax ))
    {
        ...
        std::string fn = std::to_string(stamp.sec);

        cv::Mat rgbMat;
        if (rgb->height >= 0 and rgb->width >= 0){
            cv::Mat(rgb->height, rgb->width, CV_8UC4, rgb->data).copyTo(rgbMat);
            cv::imwrite("./images/" + fn + ".bmp", rgbMat);
            ros::Duration(10).sleep();
        }

        img.header.seq = rgb->sequence;
        img.header.stamp = ros::Time::now(); // rgb->timestamp;
        img.header.frame_id = "table_frame";
        ...
        //pcl::io::savePCDFileASCII (fn + ".pcd", *cloud);
